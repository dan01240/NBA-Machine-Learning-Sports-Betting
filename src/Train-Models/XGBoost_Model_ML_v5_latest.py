...
class XGBoostModelTrainer:
    ...
    def train_and_evaluate(self, num_iterations=1000):
        ...
        for iteration in tqdm(range(num_iterations)):
            ...
            # XGBoost_Runnerと互換性のあるモデルパラメータ
            param = {
                'max_depth': 3,
                'eta': 0.01,
                'objective': 'multi:softprob',  # 重要: 確率配列として出力するために変更
                'num_class': 2,                 # 重要: 2クラス（アウェイチーム勝利=0, ホームチーム勝利=1）
                'eval_metric': 'mlogloss',      # マルチクラス用に変更
                'subsample': 0.8,
            ...
            # バリデーションデータでの予測
            y_pred_proba_raw = model.predict(dval)
            
            # 確率の抽出（multi:softprobはクラスごとの確率を返す）
            y_pred_proba = y_pred_proba_raw[:, 1]  # インデックス1がホームチーム勝利の確率
            ...
            if cal_metrics['calibration_error'] < self.best_cal_error:
                ...
                # XGBoost_Runner用のモデル出力のテスト
                test_sample = dval.slice([0, 1])  # サンプル用にデータを2つ取得
                test_pred = model.predict(test_sample)
                print(f"テスト予測の形状: {test_pred.shape}, 値: {test_pred}")
                print("XGBoost_Runnerと互換性のある予測形式（各行は[アウェイ勝率, ホーム勝率]）")
                
                # XGBoost_Runnerのコードに合わせた形式でアクセスのテスト
                try:
                    away_win_prob = test_pred[0][0]
                    home_win_prob = test_pred[0][1]
                    print(f"テストサンプル1 - アウェイ勝率: {away_win_prob:.4f}, ホーム勝率: {home_win_prob:.4f}")
                    print("↑ この形式が XGBoost_Runner.py で必要とされる形式です")
                except Exception as e:
                    print(f"モデル出力のアクセスに問題があります: {e}")
    ...
    def evaluate_on_holdout_data(self):
        ...
        holdout_pred_proba_raw = self.best_model.predict(dholdout)
        
        # 確率の抽出（multi:softprobはクラスごとの確率を返す）
        holdout_pred_proba = holdout_pred_proba_raw[:, 1]  # インデックス1がホームチーム勝利の確率
